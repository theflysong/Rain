...
namespace rain {
    ...
    public:
        ...
        static ParseResult<Terminal> parse(TokenIter begin, TokenIter end) {
            Token *tok = *begin;
            if (begin != end && (*begin)->type == T) {
                return ParseResult<Terminal>(true, new Terminal(*begin), begin + 1);
            }
            return ParseResult<Terminal>::failed(end);
        }
    };
    ...
    class Closure {
    public:
        Closure(std::vector<T*>&& children) : _children(std::move(children)) {}
        ...
        static ParseResult<Closure<T>> parse(TokenIter begin, TokenIter end) {
            ...
            return ParseResult<Closure<T>>(true, new Closure<T>(std::move(children)), current);
    }
    ...
    class Connect {
    public:
        Connect(std::tuple<Nodes*...>&& children) : _children(std::move(children)) {}
        ...
        static ParseResult<Connect> parse(TokenIter begin, TokenIter end) {
    ...
    private:
        ...
        static ParseResult<Connect> parse_impl(TokenIter begin, TokenIter end, std::tuple<ParsedNodes...>&& current_tuple) {
            if constexpr (Index == sizeof...(Nodes)) {
                // 所有节点都解析成功，创建Connect
                return ParseResult<Connect>(
                    true, 
                    new Connect(std::move(current_tuple)),
                    begin
                );
            } else {
                // 提取现在应该使用的Node
                ...
                if (!result.success) {
                    return ParseResult<Connect>::failed(begin);
                }
    ...
    class Choice {
    public:
        Choice(std::variant<Nodes*...>&& child, size_t index)
        ...
        static ParseResult<Choice> parse(TokenIter begin, TokenIter end) {
    ...
    private:
        ...
        template<size_t Index>
        static ParseResult<Choice> parse_impl(TokenIter begin, TokenIter end) {
            if constexpr (Index == sizeof...(Nodes)) {
                return ParseResult<Choice>::failed(end);
            } else {
                using CurrentType = std::tuple_element_t<Index, std::tuple<Nodes...>>;
                ...
                if (result.success) {
                    ...
                    return ParseResult<Choice>(true, new Choice(std::move(v), Index), result.end);
                }
    ...
    TypeNode
    >;
    class TypeNode : public Choice<
        Connect<
            Terminal<TokenType::SIGN_STAR>,
...
int main(int argc, char** argv) {
    std::vector<rain::Token *> tokens = {
        new rain::Token(rain::TokenType::SIGN_STAR, "*"),
        new rain::Token(rain::TokenType::SIGN_POINTER, "->"),
        new rain::Token(rain::TokenType::SIGN_STAR, "*"),
        new rain::Token(rain::TokenType::ENDMARK, "$")
    };

    auto res = rain::TypeNode::parse(tokens.begin(), tokens.end());
    if (res.success) {
